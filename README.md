...
### 고치면 파일에 씁니다 (write-back)

텔레그램에서 "다음할일은 ~", "중단할게" 라고 하면 DB가 아니라 **`_STATUS.md`가
고쳐집니다.** DB만 고치면 다음 동기화가 파일 내용으로 덮어써서, 사용자에게는
저장된 것처럼 보이는데 실제로는 사라집니다.

```
update_project()
   ├ status_path 있음 → _STATUS.md 수정 → render.py 재생성 → 검증 → sync()
   └ status_path 없음 → 예전대로 DB 수정 (손으로 만든 프로젝트)
```
...
쓰고 나면 `updated:`를 오늘 날짜로 올립니다. 사람이 손으로 고칠 때 하는 것과
같습니다. `status:` 뒤에 붙은 주석은 그대로 둡니다.

**검증이 핵심입니다.** 프론트매터를 고치는 건 텍스트 수술이지 파싱이 아닙니다.
우리가 쓴 것과 `render.py`가 읽는 것이 같다는 보장은, 쓴 뒤에 색인을 다시
만들어 **그 단일 파서가 무엇을 읽었는지 확인하는 것**뿐입니다. 어긋나면
파일을 원래대로 되돌리고 `WriteBackError`를 올립니다. 쓰기는 같은 디렉터리에
임시 파일로 한 뒤 바꿔치기하므로, 중간에 죽어도 반쯤 쓰다 만 `_STATUS.md`는
남지 않습니다.

`WriteBackError`가 나면 **DB는 건드리지 않고** 사용자에게 그대로 알립니다.
저장된 척하지 않습니다.

되돌릴 수 없는 정보가 하나 있습니다. Marvis의 `중단`은 SECRETARY의
관찰중/멈춤/종료가 합쳐진 것이라, `sub_status`가 그 셋 중 하나일 때만 그대로
쓰고 아니면(`일시정지` 같은 Marvis 고유 태그) `멈춤`으로 적습니다. 그래서
응답은 요청한 말이 아니라 **실제로 파일에 남은 값**을 알려줍니다.

### 갑자기 떠오른 것은 따로 적습니다

`next:` 는 하기로 정리된 계획입니다. 샤워하다 떠오른 착상을 거기 넣으면 둘이
섞여서 아침 브리핑이 "정해진 일"인지 "그냥 든 생각"인지 구분되지 않습니다.

그래서 본문 맨 아래에 별도 절을 둡니다.

```markdown
## [텔레그램 전송]
<!-- 진웅이 텔레그램으로 그때그때 보낸 것.
     코드나 git 이력에서 나온 내용이 아니라 즉흥적으로 떠오른 착상이다.
     ... 여기 있다는 이유만으로 합의된 계획이 아니다. -->
- 2026-09-03 10:58 — 유저정보 수집
```

주석은 나중에 이 파일을 여는 사람이나 에이전트(Claude Code, Codex)를 위한
표지입니다. 마크다운 렌더링에는 안 보이고 원문을 읽는 쪽에는 반드시 보입니다.
`_STATUS.md` 의 다른 내용은 코드와 커밋에서 나온 사실이지만 이 절은 아니라는
것을, 읽는 쪽이 물어보지 않고도 알아야 합니다.

```
텔레그램: "소곤.zip 아이디어 유저정보 수집"
       → '소곤.zip (Sogon.zip)' _STATUS.md에 적었습니다.
         - 2026-09-03 10:58 — 유저정보 수집
```

* **`프로젝트` 라는 낱말을 요구하지 않습니다.** 갑자기 떠올라 던지는 말에
  격식을 요구하면 결국 안 쓰게 됩니다. 대신 **이름이 실제 프로젝트로 풀릴 때만**
  이 경로로 갑니다. "좋은 아이디어가 있어" 는 여기 걸리지 않습니다.
* 키워드는 `아이디어` 또는 `메모`, 뒤에 `:` 는 있어도 없어도 됩니다.
* **덧붙이기만 합니다.** 기존 항목을 지우거나 바꾸지 않습니다.
* **`updated:` 를 올리지 않습니다.** 떠오른 걸 적어둔 것은 상태 갱신이 아니고,
  올리면 대시보드의 "며칠 전" 이 거짓말이 됩니다. 대신 항목마다 날짜를 답니다.
* 응답은 "적어뒀습니다" 가 아니라 **파일에 실제로 적힌 줄을 그대로** 돌려줍니다.
* `render.py` 는 이 절을 읽지 않습니다(프론트매터와 `## 한 줄 요약` 만 봅니다).
  그래서 검증도 색인이 아니라 파일을 다시 읽어서 합니다.

정리해서 `next` 로 올릴지는 사람이 판단합니다. Marvis 는 옮기지 않습니다.
